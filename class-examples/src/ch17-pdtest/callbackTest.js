//확장팩 코드 러너 설치 후.
// js 바닐라 js 기본 js 실행 하는 방법 ctrl + alt +n 
// 비동기 통신 관련된, 콜백 함수라, async, await, promis 간단 설명.
...
// 그냥 함수를 막 실행해요. non-blocking, 
// 대신에 해당 함수가 끝났다라는 신호를 보내게 됩니다. 콜백함수. 상황을 보고를 받는다. 
...
//     }, 2000)
// }
// setTimeout ( 콜백함수, 시간(ms)) : 2번째 매개변수 시간 후에, 첫번째 매개변수 함수가 호출됨. 
// ex1
// setTimeout(print,3000);
//실행 순서, 대기중 먼저 뜨고 -> 3초 뒤에 -> hi 리액트 
...
// 비동기 작업을 위해서 -> 콜백 함수 도입 문제점(콜백지옥)-> es6 (Promise) 문법을 -> 조금더 가독성있게 작업을 하게됨. 

//ex4 Promise 문법으로 변경. 
function increase2(number) {
    // increase2  함수에서 성공시, resolve 함수를 반환, 실패시, reject 함수를 반환. 
...
// 함수 앞에 async , await 는 Promise 를 반환하는 함수 앞에 붙인다. 

// axios : 통신 라이브러리 사용할 예정임, 이 라이브러리는 반환을 Promise 타입으로 반환을 합니다. 
// 더미 데이터를 axios 를 이용해서 , 받아오고, 
